...
export default class GeniusLyrics {
   ...
   */
  public readonly auris: GeniusAurisLink

  /**
   * Creates a new Genius lyrics provider instance.
   *
   * @param auris - Runtime container passed by the lyrics manager.
   */
  public constructor(auris: GeniusAurisLink) {
    this.auris = auris
  }
  ...
  private extractPreloadedState(html: string): GeniusPreloadedState | null {
    ...
    try {
      ...
      log('debug', 'Lyrics', `Failed to parse Genius page state: ${message}`)
      return null
    }
  }
  ...
  public async getLyrics(trackInfo: TrackInfo): Promise<LyricsResult> {
    ...
    log('debug', 'Lyrics', `Searching Genius for: ${query}`)
    ...
    try {
      ...
      const songPageResponse = await httpGet(
