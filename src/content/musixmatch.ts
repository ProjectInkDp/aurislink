...
import type { TrackInfo } from '../typings/engine/player.js';
...
export default class MusixmatchLyrics {
  private auris: AurisInstanceForMusixmatch
  private guid: string
  ...
  constructor(auris: AurisInstanceForMusixmatch) {
    this.auris = auris
    this.guid = _guid()
  ...
  async setup(): Promise<boolean> {
    const signatureSecret =
      this.auris.options.lyrics?.musixmatch?.signatureSecret
  ...
  private _signUrl(url: string): string {
    const secret = this.auris.options.lyrics?.musixmatch?.signatureSecret
  ...
  private async _saveToken(token: string, expires: number): Promise<void> {
    try {
      ...
      log('warn', 'Lyrics', `Failed to save token: ${message}`)
    }
  }
  ...
  async getLyrics(trackInfo: TrackInfo): Promise<LyricsResult> {
    try {
      ...
      if (cached !== undefined) {
        log('debug', 'Lyrics', 'Cache hit')
        return cached
      }

      log('info', 'Lyrics', `Searching: "${title}" by "${artist}"`)
      ...
      const message = e instanceof Error ? e.message : String(e)
      log('error', 'Lyrics', `Failed: ${message}`)
      return {
        loadType: 'error',
