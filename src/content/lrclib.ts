...
export default class LRCLIBLyrics {
   ...
   */
  public readonly auris: LRCLibAurisLink
   ...
   */
  public constructor(auris: LRCLibAurisLink) {
    this.auris = auris
  }
  ...
  public async getLyrics(trackInfo: TrackInfo): Promise<LyricsResult> {
    ...
    log('debug', 'Lyrics', `Searching LRCLIB for: ${query}`)
    ...
    try {
      ...
      if (response.status !== 200) {
        throw new Error(`Unexpected LRCLIB status code: ${response.status}`)
      }
