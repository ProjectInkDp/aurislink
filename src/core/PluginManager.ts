...
import type { 
  ...
  PluginMeta,
  PluginExecutor 
} from '../typings/core/plugin.js';

/**
 * PluginManager for AurisLink.
 * Supports loading plugins from Local, NPM, GitHub, and direct URLs.
 * Follows the zero-dependency philosophy where possible, using native Node.js features.
 */
export class PluginManager {
  ...
  public async setup(): Promise<void> {
    ...
    log('info', 'PluginManager', `Initializing ${plugins.length} plugins...`);

    try {
      await fs.mkdir(this.pluginsDir, { recursive: true });
    } catch (err) {
      log('error', 'PluginManager', `Failed to create plugins directory: ${err}`);
      return;
    }
    ...
    for (const pluginDef of plugins) {
      try {
        ...
        log('error', 'PluginManager', `Failed to load plugin "${pluginDef.name}": ${err}`);
      }
    }
  }
  ...
  private async _loadPlugin(def: PluginDefinition): Promise<void> {
    ...
    const context: PluginContext = {
      ...
      meta,
      logger: (level, message) => log(level, `Plugin:${def.name}`, message),
    };

    await module.default(context);

    this.loadedPlugins.set(def.name, meta);
    log('info', 'PluginManager', `Loaded plugin: ${meta.name} v${meta.version}${meta.author ? ` by ${meta.author}` : ''}`);
  }
  ...
  private async _resolveNpm(def: PluginDefinition, meta: PluginMeta): Promise<string> {
    ...
    log('debug', 'PluginManager', `Resolving NPM package: ${pkgName}`);
    
    try {
      // Try to find it in node_modules
      const resolvedPath = import.meta.resolve(pkgName);
      const entryPoint = new URL(resolvedPath).pathname;
      
      // Try to find package.json to get meta
      ...
      log('info', 'PluginManager', `NPM package ${pkgName} not found, attempting to install...`);
  ...
  private async _resolveGitHub(def: PluginDefinition, meta: PluginMeta): Promise<string> {
    ...
    if (!existsSync(targetDir)) {
      log('info', 'PluginManager', `Cloning GitHub plugin: ${repo}${branch ? `#${branch}` : ''}`);
      ...
      if (existsSync(path.join(targetDir, 'package.json'))) {
        log('debug', 'PluginManager', `Installing dependencies for ${def.name}...`);
        execSync('npm install --production', { cwd: targetDir });
      }
    } else {
      log('debug', 'PluginManager', `GitHub plugin ${def.name} already exists, skipping clone.`);
    }
  ...
  private async _resolveUrl(def: PluginDefinition, meta: PluginMeta): Promise<string> {
    ...
    log('info', 'PluginManager', `Downloading plugin from URL: ${url}`);
