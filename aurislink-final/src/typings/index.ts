...
// ─── Declaração de plugin no config.ts ───────────────────────────────────────
...
export interface PluginDeclaration {
   ...
   *  - `'url:https://example.com/plugin.js'`   — URL direta (arquivo único)
   */
  dependency: string
  /** Configurações livres passadas ao plugin via ctx.pluginConfig */
...
export interface AurisConfig {
  ...
  /** Lista de plugins a carregar no boot */
