...
export default class YandexMusicLyrics {
  private readonly tokenStore: Vault
  private accessToken: string | null = null

  constructor(tokenStore: Vault) {
    this.tokenStore = tokenStore
  }

  /** Load the access token from config-injected store or Vault cache. */
  async setup(accessToken?: string): Promise<boolean> {
    ...
    // Fall back to the encrypted Vault (survives restarts)
